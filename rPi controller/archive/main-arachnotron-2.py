...
def draw_eyes(data=None, blink = 0.0, color = (90,210,252)):
    startTime = time.time()
    gait = 0
    ...
    uiPhaseStartColor = uiAccentColor
    ...
    eye_image = Image.new("RGB", (eye_width,eye_height), "BLACK")
    eye_draw = ImageDraw.Draw(eye_image)
    height_offset = eye_height * blink
    eye_draw.ellipse((0, 0, eye_width, eye_height*blink), fill=color, outline=color)
    # create a new blank image
    image1 = Image.new("RGB", (disp.height, disp.width), "BLACK")
    draw = ImageDraw.Draw(image1)
    middle = ((disp.height - 2) * .25, (disp.width - 2) * .45)
    #color = (90,210,252)
    eye_height = 34
    eye_width = 40
    draw.ellipse((tuple(map(operator.sub, middle, (eye_width,eye_height * blink))), tuple(map(operator.add, middle, (eye_width,eye_height * blink)))), fill=color, outline=color)
    #draw.ellipse((tuple(map(operator.sub, middle, (4,4))), tuple(map(operator.add, middle, (4,4)))), fill="BLACK", outline="BLACK")
    middle = (disp.height - ((disp.height - 2) * .25), disp.width - ((disp.width - 2) * .55))
    draw.ellipse((tuple(map(operator.sub, middle, (eye_width,eye_height * blink))), tuple(map(operator.add, middle, (eye_width,eye_height * blink)))), fill=color, outline=color)
    #draw.ellipse((tuple(map(operator.sub, middle, (4,4))), tuple(map(operator.add, middle, (4,4)))), fill="BLACK", outline="BLACK")

    # # Draw the eye lids
    # for i in range(5, int(eye_height * blink)):
    #     #draw.arc((tuple(map(operator.sub, middle, (eye_width,eye_height - (eye_height * blink) + i))), tuple(map(operator.add, middle, (eye_width,eye_height - (eye_height * blink) + i)))), 0, 180, fill="BLACK", width=1)
    #     draw.arc((tuple(map(operator.sub, middle, (eye_width+1,eye_height + i - (eye_height * blink)))), tuple(map(operator.add, middle, (eye_width+1,eye_height + i - (eye_height * blink))))), 0, 180, fill="GRAY", width=2)
    #     draw.arc((tuple(map(operator.sub, middle, (eye_width+1,eye_height + i - (eye_height * blink)))), tuple(map(operator.add, middle, (eye_width+1,eye_height + i - (eye_height * blink))))), 180, 360, fill="BLACK", width=2)
    # draw lines through the eyes to simulate VFD type display
    for i in range(int(middle[1] - eye_width), int(middle[1] + eye_width),3):
        draw.line((1, i, disp.height, i), fill="BLACK", width=2)
            ...
            while True:
                ...
                if (port.in_waiting > 0):
                    ...
                    try:
                        ...
                        startTime = time.time()
                        # read the telemetry line
                        telemLine = port.readline().strip()
                        # decode the telemetry line and split it into data segments
                        dataSegments = telemLine.decode("ascii").strip().split('|')

                        image1, gait = draw_eyes(dataSegments, blink, eye_color)
                        blink += dir * 0.015
                        if (blink > 1.0):
                            dir = -1.0
                            eye_color = (10,150,252)
                        if (blink < 0.0):
                            dir = 1.0
                            if (random.random() > 0.8):
                                eye_color = "RED"
                        #image1, gait = draw_screen(dataSegments)
                        image2 = np.array(image1)
                        height, width = image2.shape[:2]
                        image2 = cv.resize(cv.cvtColor(image2, cv.COLOR_RGB2BGR), (width*2, height*2), interpolation=cv.INTER_LANCZOS4)
                        cv.imshow("arachnotron", image2)
                        cv.waitKey(1) 
                        #print(time.time() - displayTimer)
                        #print("display time = ", (time.time() - displayTimer) * 1000, "ms")
                        if (time.time() - displayTimer) * 1000 > 100:
                            displayTimer = time.time()
                            dispStart = time.time()
                            disp.ShowImage(image1) 
                            #print("ShowImage time = ", (time.time() - dispStart) * 1000, "ms") 
                        endTime = time.time()
                        #print("read telemetry time = ", (endTime - startTime) * 1000.0, "ms")
