...
def draw_screen(data = None):
        ...
        for dLine in data:
            ...
            if (len(dLine)):
                ...
                if (header == "S1"):
                    ...
                    currentGait = getGait(int(dataElements[6]))
                    ...
                    if (int(dataElements[3]) == 1):
                        #draw.text((100, 60), "MOTORS", fill = uiBorderColor, font=fontMICR_14pt)
                        #draw.text((105, 78), "ON", fill = "GREEN", font=fontMICR_14pt)
                        color = "GREEN"
                    else:
                        #draw.text((100, 50), "MOTORS", fill = uiBorderColor, font=fontMICR_14pt)
                        #draw.text((105, 78), "OFF", fill = "RED", font=fontMICR_14pt)
                        color = "RED"
                    ...
                    for i in range(0, 18):
                        ...
                        if (float(dataElements[i+18]) > 30.0):
                            color = "RED"
                        else:
                            color = uiBorderColor
    ...
    endTime = time.time()
    print("draw_screen time = ", (endTime - startTime) * 1000.0, "s")
    return image1
            ...
            while True:
                ...
                if (port.in_waiting > 0):
                    ...
                    try:
                        ...
                        #print(dataSegments.__len__())

                        image1 = draw_screen(dataSegments)
                        image2 = np.array(image1)
                        height, width = image2.shape[:2]
                        ...
                        if (time.time() - displayTimer) * 1000 > 500:
                            displayTimer = time.time()
                            dispStart = time.time()
                            disp.ShowImage(image1) 
                            print("ShowImage time = ", (time.time() - dispStart) * 1000, "ms") 

                        # loop through the data segments and process them
                        # for dLine in dataSegments:
                        ...
                        #         # split out the segment data elements
                        #         dataElements = dLine.split(':')[1].split(',')
                                #print(header, dataElements)

                                # process the segment data elements
                                #  Serial << "S1:" << _mainLoop.DeltaT() << "," << _FLOAT(_bodyCentroid.x, 1) << "," << _FLOAT(_bodyCentroid.y, 1) << "," << _state.MotorEnabled << "," << _state.LastGait << "," << _state.NewGait << ",";
                                # Serial << _state.CurrentGait << "," << _FLOAT(_state.Phase, 5) << "," << _state.CurrentReadServo << "|";  Serial << "S1:" << _mainLoop.DeltaT() << "," << _FLOAT(_bodyCentroid.x, 1) << "," << _FLOAT(_bodyCentroid.y, 1) << "," << _state.MotorEnabled << "," << _state.LastGait << "," << _state.NewGait << ",";
                                # image1 = draw_screen(dataElements)
                                # image2 = np.array(image1)
                                # height, width = image2.shape[:2]
                                # image2 = cv.resize(cv.cvtColor(image2, cv.COLOR_RGB2BGR), (width*2, height*2), interpolation=cv.INTER_LANCZOS4)
                                # cv.imshow("arachnotron", image2)
                ...
                #print(" trigger_right = ", round(remote_control.trigger_right,2), "  joystick_right_x = ", round(remote_control.joystick_right_x,3), "  joystick_right_y = ", round(remote_control.joystick_right_y,3))
